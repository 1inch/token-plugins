...
- **High security**: 1inch Token Hooks have gone through extensive [audits](https://github.com/1inch/1inch-audits/tree/master/Fusion%20mode%20and%20Token-plugins) by multiple top-tier companies.
...
## Use-Cases
Here are some examples of how Token Hooks is currently being (or could be used) today:
...
![ERC20Hooks](/src/img/PluginsDiagram.png)
...
Below is an example of an ERC20 token implementing hook support and a simple hook that mints and burns its own token based on the parent’s token balance.

**Simple hook-enabled token contract**
```
contract NewToken is ERC20Hooks {
